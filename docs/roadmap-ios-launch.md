...
**Status (2026-07-02): DONE, deployed, hermetically verified.** `docs/moderation-runbook.md` shipped (list/release/reject held reports, check reporter history, remove an abusive account via the same aggregate-then-delete flow as self-service deletion). `DELETE /api/reports/{id}` (2-minute undo window, server-enforced independent of frontend toast timing) live, confirmed 401-gated correctly. Local Guide read/render path shipped — `get_reports_for_beach` now computes `is_local_guide` per report server-side (never exposes `reporter_id` itself), `CommunityReports` renders 🏅 when set; reads `false` for everyone today since Phase C's promotion job isn't built, by design — the render path just needs to exist now so C is a backend-only change later. **Needs a live signed-in test to close out** (same boundary as every other auth-required flow this session): submit a report, tap Undo within 8s, confirm it's gone from the community list and `reports` table.
