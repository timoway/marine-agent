...
- **Beach list grew 21 → 40** (owner request, reopening the earlier "focus the 21" decision at their explicit direction): 5 Gulf gaps filled in already-covered regions (Naples Pier, Tigertail + South Marco on Marco Island, **Brohard Paw Park** and **Honeymoon Island Dog Beach** — see dog-beach research below) plus **14 Atlantic coast beaches** (Jacksonville Beach → Miami Beach), added at "full comprehensive coverage" per owner's answer when asked to scope it. Every tide/NWS station was verified live against NOAA CO-OPS and api.weather.gov (not guessed) before merging; Mote IDs confirmed via Mote's own GraphQL API. A `coast` field (gulf/atlantic) now actually backs the `/api/rank` `coast` param that previously existed but did nothing.

**Data hardening decision (owner question, 2026-07-02): OpenStreetMap over Google Places.** Tested both approaches directly rather than choosing on priors:
- **OSM (Nominatim name-search + direct tag pull)** — free, no API key/billing, and it has *structured tags built for exactly this data* (`leisure=dog_park`, `fee=yes/no`). Proved itself in this session: found "Honeymoon Island Dog Beach" as its own confirmed OSM POI, and in the process **caught a real bug** — Brohard's hand-estimated coordinates were off by ~1,300 ft, corrected via OSM's surveyed location.
- **Google Places** — considered and set aside for this use case: costs money (new billing relationship) and doesn't have a reliably structured "dog policy" or "parking fee" field for beaches specifically — that info usually lives in freeform reviews, not attributes. Not clearly better for *this* problem, just more expensive.
- **Caveat found in the same test**: OSM's coverage is patchy, not a silver bullet — a blind radius search around Brohard's (wrong) coordinates returned nothing, and two other dog-beach candidates (Bunche Beach Preserve, Fred Howard Park) are real public natural areas with no dog-park tag confirmable in OSM. So the real pattern is: **use OSM as a free, offline, per-beach verification pass** (query a specific candidate by name, pull its tags, cross-check against the curated `BEACH_AMENITIES` first pass) — not a live runtime dependency (these facts don't change minute-to-minute like weather does, so no need to fetch them on every request) and not a substitute for human judgment on ambiguous cases.

**More Gulf dog beaches (owner question, 2026-07-02):** researched via the OSM method above rather than relying on memory. Found one solid addition — **Honeymoon Island Dog Beach** (Dunedin, Pinellas County), confirmed via its own `leisure=dog_park` OSM tag, added with a real Mote/tide/NWS wiring (mote_id reuses adjacent Caladesi Island State Park, same barrier-island system) and live-verified. Two other named candidates I considered (Bunche Beach Preserve in Lee County, Fred Howard Park in Pinellas) are real, well-known public natural areas near the Gulf but have **no confirmable dog-beach designation in OSM** — not added rather than guessed; worth the owner's local knowledge if either is actually dog-friendly and just under-mapped.
- `BEACH_AMENITIES` (parking/dog_friendly/dog_notes/restrooms/lifeguard) added for all 40 beaches, injected into `/api/conditions` as `amenities`. **This data is a first pass from general public knowledge, not live-verified the way the station IDs were** (fees/policies drift in ways no API check catches) — flagged in code; owner should spot-check before fully relying on it, per the original "owner curates" plan this pulled forward from.
...
**Customizable rank filters (owner question, 2026-07-02) — recommendation: persist selection now, defer more filter types/full customization.** Today's two chips (`rankDogFriendly`/`rankFreeParking` in `App.tsx`) reset every session. The cheap, high-value next step — same pattern as favorites — is **persisting the chosen filters** (localStorage is enough; doesn't need the server-side favorites treatment since it's a display preference, not data other features build on) so a dog owner doesn't re-toggle it every visit. I'd hold off on adding *more* filter types (lifeguard-required, low-crowd, etc.) or a full custom-weighting UI until real usage data shows which filters people actually reach for — that's exactly the "decide with data" instinct Track 5's analytics exists for, and guessing at more filters now risks the same over-building this whole plan has been deliberately avoiding. **Shipped same day**: persistence, plus two follow-ons the owner surfaced. (1) Filtered rank results now sort by distance (not condition-tier) with `limit=25` instead of 5, since "which nearby beaches have X" is a browse-by-distance question — `rank_beaches_data`/`GET /api/rank` gained `sort_by`/`sort` (`"condition"` default, unchanged; `"distance"`). Verified: owner's "only one dog beach within 50mi of Manasota Key" observation was confirmed accurate (Brohard 4.4mi, Honeymoon Island 76.2mi — outside the default radius, not a bug). (2) Owner then asked to confirm exactly what changed — correctly suspected the radius/activity behavior hadn't actually moved — and chose to remove the radius cap entirely when filtering (`radius_miles=1000` instead of 50; no backend change needed, `GET /api/rank` already accepted any value). Both dog beaches now appear from Manasota Key, correctly ordered; default (unfiltered) "best nearby today" behavior is untouched at 50mi/condition-sort/limit 5.

**Flag color provenance (owner question, 2026-07-02):** the official flag/verdict color (`calculate_flag` in `marine_server.py`) is driven **entirely by official data** — Mote's jellyfish report field (purple), wave/wind observations, and FWC red tide status (red/double-red). Beach Pulse community reports never feed into it — confirmed both by code (`jellyfish_present` is read from `mote.get("jellyfish")`, never from `reports`/`beach_pulse`) and live data: Pensacola was showing `PURPLE FLAG` with Mote reporting `jellyfish: "Some"` *and*, separately, one unescalated community jellyfish report from 7 minutes prior in `beach_pulse` — a coincidental confirmation of the official signal, not its cause. This is the "verdict never rewritten by reports" design principle working as intended.
...
**Status (2026-07-02): DONE — fully closed out, owner confirmed Undo works live.** `docs/moderation-runbook.md` shipped (list/release/reject held reports, check reporter history, remove an abusive account via the same aggregate-then-delete flow as self-service deletion). `DELETE /api/reports/{id}` (2-minute undo window, server-enforced independent of frontend toast timing) live and **verified end-to-end by the owner** (submit → Undo → confirmed removed). Local Guide read/render path shipped — `get_reports_for_beach` computes `is_local_guide` per report server-side (never exposes `reporter_id`), `CommunityReports` renders 🏅 when set; reads `false` for everyone today since Phase C's promotion job isn't built, by design.
